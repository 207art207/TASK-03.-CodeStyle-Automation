...
static ssize_t ext4_file_splice_read(struct file* in,
                                     loff_t* ppos,
 ...
 */
static bool ext4_unaligned_io(struct inode* inode,
...
static ssize_t ext4_handle_inode_extension(struct inode* inode,
                                           loff_t offset,
                                           ssize_t written,
                                           ssize_t count) {
  handle_t* handle;
...
static int ext4_dio_write_end_io(struct kiocb* iocb,
                                 ssize_t size,
                                 int error,
                                 unsigned int flags) {
  ...
  if (!error && size && (flags & IOMAP_DIO_UNWRITTEN) &&
    ...
    error = ext4_convert_unwritten_extents_atomic(NULL, inode, pos, size);
 ...
 */
static ssize_t ext4_dio_write_checks(struct kiocb* iocb,
                                     struct iov_iter* from,
                                     bool* ilock_shared,
                                     bool* extend,
                                     int* dio_flags) {
