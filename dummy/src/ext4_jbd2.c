...
handle_t* __ext4_journal_start_sb(struct inode* inode,
  ...
  int err;
  if (inode)
    trace_ext4_journal_start_inode(inode, blocks, rsv_blocks, revoke_creds,
                                   type, _RET_IP_);
  else
    trace_ext4_journal_start_sb(sb, blocks, rsv_blocks, revoke_creds, type,
                                _RET_IP_);
...
handle_t* __ext4_journal_start_reserved(handle_t* handle,
  ...
  trace_ext4_journal_start_reserved(sb, jbd2_handle_buffer_credits(handle),
                                    _RET_IP_);
  err = ext4_journal_check_start(sb);
  if (err < 0) {
...
static void ext4_journal_abort_handle(const char* caller,
                                      unsigned int line,
                                      const char* err_fn,
...
int __ext4_forget(const char* where,
  ...
  ext4_debug("forgetting bh %p: is_metadata=%d, mode %o, data mode %x\n", bh,
             is_metadata, inode->i_mode, test_opt(inode->i_sb, DATA_FLAGS));
...
int __ext4_handle_dirty_metadata(const char* where,
  ...
  if (ext4_handle_valid(handle)) {
    ...
    if (inode)
      mmb_mark_buffer_dirty(bh, &EXT4_I(inode)->i_metadata_bhs);
    else
      mark_buffer_dirty(bh);
    if (inode && inode_needs_sync(inode)) {
      sync_dirty_buffer(bh);
      if (buffer_req(bh) && !buffer_uptodate(bh)) {
        ext4_error_inode_err(inode, where, line, bh->b_blocknr, EIO,
