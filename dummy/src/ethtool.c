...
static int e1000_set_link_ksettings(struct net_device* netdev,
  ...
  if (cmd->base.eth_tp_mdix_ctrl) {
    ...
    if ((cmd->base.eth_tp_mdix_ctrl != ETH_TP_MDI_AUTO) &&
      ...
      e_err(
          "forcing MDI/MDI-X state is not supported when link speed and/or "
          "duplex are forced\n");
      return -EINVAL;
    }
  }
...
static bool reg_pattern_test(struct e1000_adapter* adapter,
                             u64* data,
                             int reg,
                             int offset,
                             u32 mask,
...
static bool reg_set_and_check(struct e1000_adapter* adapter,
                              u64* data,
                              int reg,
                              u32 mask,
                              u32 write) {
...
static void e1000_get_wol(struct net_device* netdev,
  ...
  if (adapter->flags & FLAG_NO_WAKE_UCAST) {
    ...
    if (adapter->wol & E1000_WUFC_EX)
      e_err(
          "Interface does not support directed (unicast) frame wake-up "
          "packets\n");
  }
...
static int e1000e_get_ts_info(struct net_device* netdev,
  ...
  info->rx_filters =
      (BIT(HWTSTAMP_FILTER_NONE) | BIT(HWTSTAMP_FILTER_PTP_V1_L4_SYNC) |
       ...
       BIT(HWTSTAMP_FILTER_PTP_V2_EVENT) | BIT(HWTSTAMP_FILTER_PTP_V2_SYNC) |
       BIT(HWTSTAMP_FILTER_PTP_V2_DELAY_REQ) | BIT(HWTSTAMP_FILTER_ALL));
