...
usage() {
    ...
    echo "  $0 <project-dir>       - make project formatting with clang-format-22 version"
    echo
    echo "Example:"
    echo "  $0 ./dummy"
    echo
...
formater_check_cust() {
    echo "==== CLANG-FORMAT ${CF_YY} VERSION ===="

    if ! ${CF_YY} --style=file --dump-config </dev/null >/dev/null; then
        echo "[error] Configuration is incompatible or invalid." >&2
        echo
        RESULT=2
    elif find . -type f \( -name '*.c' -o -name '*.h' \) \
        -exec ${CF_YY} --style=file --dry-run --Werror --ferror-limit=1 {} +; then
